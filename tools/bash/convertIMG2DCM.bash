...
function convertIMG2DCM {
  ...
  local ref_dcm_dir=$(dirname "${ref_dcm}")
  local no_of_orig_dcms=$(wc -l "${ref_dcm_dir}/../dcm-in-index" | cut -d" " -f1)
  ...
  if [[ "$replace_study_description" != "" ]]; then
    do_replace_study_description="--key 'StudyDescription=${do_replace_study_description}'"
  fi
  ...
  if [[ $no_of_imgs != $no_of_orig_dcms ]]; then
    warning "    The number of original DICOM files and images to be converted differs (${no_of_imgs} != ${no_of_orig_dcms}). We are assuming that the original images are of non-axial orientation. Some position information will not be transferred."
    ...
    cat "${ref_dcm_dir}/../dcm-in-index" | while read slice dcm; do
